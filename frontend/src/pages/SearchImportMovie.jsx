...
const SearchImportMovie = () => {
  ...
  const handleSearch = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await api.get(`/movies/tmdb/search?query=${searchQuery}`);
  ...
  const handleImport = async (tmdbId) => {
    setImporting(tmdbId);
    try {
      await api.post(`/movies/tmdb/import/${tmdbId}`);
  ...
  const handleViewDetails = async (tmdbId) => {
    try {
      const response = await api.get(`/movies/tmdb/details/${tmdbId}`);
